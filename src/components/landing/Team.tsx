...
export function Team({ config, theme }: TeamProps) {
  const { title, subtitle, description, members, columns = 3, background, spacing } = config;
      ...
      <div className="container mx-auto">
        ...
        <div className={`grid grid-cols-1 ${gridColsClass[columns]} gap-8`}>
          {members.map((member) => (
            <Card
              key={member.id}
              className="border-none shadow-sm hover:shadow-lg transition-shadow overflow-hidden"
            >
              <CardContent className="p-0">
                ...
                <div className="p-6 text-center">
                  ...
                  {member.bio && (
                    <p className="text-sm mb-4" style={{ color: textMuted, fontFamily: bodyFont }}>
                      {member.bio}
                    </p>
                  )}
